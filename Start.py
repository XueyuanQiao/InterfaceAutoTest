...
def send_mail(message, all=0, fail=0, error=0):
    ...
    msg = MIMEMultipart()
    msg['From'] = formataddr(["接口自动化测试报告", "no-reply@autotest.udesk.cn"])
    ...
    html = """\
    <html><body><h3 style="font-size:20px">----- 接口自动化测试完成 -----</h3>
    ...
    <p>报告详情请点击： <a href="http://autotest.udeskt1.com/%s">接口自动化测试报告</a>...附件预览会有乱码，请下载查看</p> 
    </body></html>
...
def upload_report(local, file):
    private_key = paramiko.RSAKey.from_private_key_file('D:\id_rsa.txt')  # 要将私钥放在这里
...
if __name__ =='__main__':
    ...
    i = 0
    if i == 0:
        runner = HTMLTestReportCN.HTMLTestRunner(stream=fp, title="接口自动化测试报告", environment="http://brazil.udesk.cn",
                                                 tester="乔雪源",
                                                 description="用例执行情况")
    else:
        runner = HTMLTestReportCN.HTMLTestRunner(stream=fp, title="接口自动化测试报告", environment="http://linapp.udeskt1.com",
                                                 tester="乔雪源",
                                                 description="用例执行情况")
    # 执行用例
